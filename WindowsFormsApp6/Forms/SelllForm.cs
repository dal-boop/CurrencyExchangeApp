...
{
    public partial class SelllForm : Form
    {
        ...
        public SelllForm()
        {
            ...
            {
                // Генерируем событие с именем выбранного банка
        ...
        private async Task LoadCurrencyRatesAsync()
        {
            ...
            {
                rates = await _raiffeisenApiService.GetCurrencyRatesAsync(false); // Продажа
            }
            else if (cmbBank.SelectedItem.ToString() == "Альфа-Банк")
            {
                rates = await _alfaBankApiService.GetCurrencyRatesAsync(false); // Продажа
            }
            else if (cmbBank.SelectedItem.ToString() == "Т-Банк")
            {
                rates = await _tinkoffApiService.GetCurrencyRatesAsync(false); // Продажа
            }
        ...
        private async void BtnCalculate_Click(object sender, EventArgs e)
        {
            ...
            currencyOperationService.SaveOperation(userId.Value, fromCurrency, toCurrency, amount, result, Commission, cmbBank.SelectedItem.ToString(), "Sell");
        }
        ...
        private void SelllForm_Load(object sender, EventArgs e)
        {
